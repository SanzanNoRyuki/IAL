...
*/
	if (s == NULL) stackError(SERR_INIT);																					// Kontrola vstupneho parametru
	else s->top = -1;																															// Inicializacia vrcholu zasobniku
}
...
*/
	return (s->top == -1) ? 1 : 0;																								// Prazdny -> 1 && Neprazdny -> 0
}
...
*/
	return (s->top + 1 == STACK_SIZE) ? 1 : 0;																		// Plny -> 1 && Nenaplneny -> 0
}
...
** funkci stackEmpty.
*/
	if (stackEmpty(s)) stackError(SERR_TOP);																			// Kontrola vstupneho parametru
	else *c = s->arr[s->top];																											// Vratenie prvku z vrcholu zasobniku
}
...
**
*/
	if (!stackEmpty(s)) s->top--;																									// Odstranenie prvku z vrcholu znizenim indexu prvkov
}
...
** funkci stackFull.
*/
	if (stackFull(s))	stackError(SERR_PUSH);																			// Kontrola plnosti zasobniku
	else {
		s->top++;																																		// Zvysenie indexu prvkov
		s->arr[s->top] = c;																													// Pridanie prvku na novy vrchol zasobniku
	}
}
